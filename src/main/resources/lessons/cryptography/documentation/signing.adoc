...
Here is a simple assignment. The public part of an RSA key is sent to you. Determine the modulus of the RSA key as a hex string, and calculate a signature for that hex string using the matching private key. Note that the private key never leaves the server, so without it the signature cannot be produced. The exercise requires some experience with OpenSSL. You can search on the Internet for useful commands and/or use the HINTS button to get some tips.
