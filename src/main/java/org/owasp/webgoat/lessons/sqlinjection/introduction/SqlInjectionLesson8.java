...
public class SqlInjectionLesson8 implements AssignmentEndpoint {
  ...
  protected AttackResult injectableQueryConfidentiality(String name, String auth_tan) {
    ...
    // Surname and TAN are bound values: a quote inside either one is data, not syntax, so the
    // WHERE clause can no longer be widened into a full table dump.
  ...
  public static void log(Connection connection, String action) {
    ...
    // The audit trail is written with bound values so that logging attacker text can never
    // itself become a second injection point.
