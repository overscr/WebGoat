...
public class SqlInjectionLesson6a implements AssignmentEndpoint {
  ...
  public AttackResult injectableQuery(String accountName) {
    // Parameterized query: last_name is bound as data, so a UNION / stacked
    // payload is treated as a literal name and can never rewrite the query.
    ...
    try (Connection connection = dataSource.getConnection()) {
      ...
      try (PreparedStatement statement =
        ...
        return executeSqlInjection(statement, usedUnion, query);
      }
    } catch (Exception e) {
      return failed(this)
  ...
  private AttackResult executeSqlInjection(
      PreparedStatement statement, boolean usedUnion, String query) {
    try {
